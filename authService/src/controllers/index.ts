...
export const refreshTok = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    let usr: IOauthUser | IUser | null = null;
    if (verifyRes) {
      usr = await User.findOne({refresh_token: refToken}) as IUser;

      if (!usr) {
        usr = await OauthUser.findOne({refresh_token: refToken}) as IOauthUser;
      }
      ...
      if (usr) {
        ...
        const payload = {
          "id": usr.id,
          "email": (usr as IUser).email || (usr as IOauthUser).username
        };
...
export const githubLoginCallback = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (githubUser) {
      ...
      const user = await OauthUser.findOne({username: githubUser.login});
      ...
      res.send({"success": {"user": {username:githubUser.login}}});
    } else {
      res.status(401).send({"error": {"message": "Incorrect username or password"}});
    }
